#' @title Generate spatial predictions using the average of the hybrid method of generalized boosted regression modeling and ordinary kriging and the hybrid method of generalized boosted regression modeling and inverse distance weighting (gbmokgbmidw)
#'
...
gbmokgbmidwpred <- function (longlat, trainx, trainy, longlatpredx, predx,
  ...
  ok.pred <- gstat::krige(var1 ~ 1, data.dev, data.pred, model = model.1,
    nmax = nmaxok, block = block)$var1.pred
  gbmokgbmidw.pred1 <- pred.gbm1 + (idw.pred + ok.pred) / 2
  gbmokgbmidw.pred <- cbind(longlatpredx, gbmokgbmidw.pred1)
  names(gbmokgbmidw.pred) <- c("LON", "LAT", "Predictions")
  gbmokgbmidw.pred
}

...
author: "Jin Li (jin.li@ga.gov.au)"
date: "`r Sys.Date()`"
...
The methods currently included in the package are:
 ...
 - two highly accurate machine learning methods: random forest (rf) and general boosting model (gbm);
 - four hybrid methods: rfidw, rfok, gbmidw and gbmok; and
 - two averaging methods: the average of rfidw and rfok (rfidwrfok), and the average of gbmidw and gbmok (gbmidwgbmok)
...
A unit, scale and data-variance independent accuracy measure, the vecv (i.e., variance explained by predictive models based on cross-validation) is  implemented in function vecv. 

A few other error measures are also available in function pred.acc, including mean error (me), mean absolute error (mae), mean squared error (mse), relative me (rme), relative mae (rmae), root mse (rmse), relative rmse (rrmse), as well as the accuracy measure, vecv.
...
```
As shown above, the format of the output predictions by the functions for generating spatial predictions is in data.frame and can be exported to relevant format as desired, such as csv or raster, for further use.
